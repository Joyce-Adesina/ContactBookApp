...
namespace ContactBookApp.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [Authorize]
        [HttpPost("{id}/image")]
        public async Task<IActionResult> UploadProfilePic(string id, IFormFile file)
        {
            var result =await _userService.UploadProfileImage(id, file);
            if (result.Data.Item1)
            {
                return Ok(new { ImageUrl = result.Data.Item2 });
            }
            return NotFound();
        }
